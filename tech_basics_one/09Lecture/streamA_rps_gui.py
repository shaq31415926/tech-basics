...
def press(user_selects):
    ...
    # user score label
    user_score_label = tk.Label(text=f"{name.get()}'s score: {user_score}",
             relief=tk.RAISED,
             bg='#4834DF',
             fg="#ffffff",
             borderwidth=5,
...
# this definition enters your second page
def main_game(root):
    global user_selection_label, comp_selection_label, message, user_score, comp_score

    clear_widgets(root)

    # this picks up the name from the entry box
    welcome_label = tk.Label(text=f"Hi {name.get()}!",
    ...
    exit_button = tk.Button(text="Close Game",
              command=root.destroy)
    exit_button.place(relx=0.8, rely=0.9)


# create a button that activates the second page/main game
play_button = tk.Button(text="Let's Play",
                        fg="black",
                        height=2,
                        width=5,
